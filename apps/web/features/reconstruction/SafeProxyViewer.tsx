...
function Model() {
  ...
  return <group rotation={[0.4, 0, -0.12]}><primitive object={scene.clone()} /><Html position={[0.18, .14, .08]} center distanceFactor={5}><span className="whitespace-nowrap border border-white/30 bg-night/90 px-3 py-1 font-mono text-[9px] uppercase tracking-[.08em] text-white">Inferred · neutral central field</span></Html></group>;
}
  ...
  return <section className={`overflow-hidden border border-white/25 ${dark ? "bg-night" : "bg-parchment"}`} data-testid="safe-proxy-viewer">
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4 text-white">
      ...
      <div className="flex gap-2">
        <button className="focus-ring inline-flex items-center gap-2 border border-white/20 px-3 py-2 font-mono text-[9px] uppercase tracking-[.08em] hover:bg-white/10" onClick={() => setDark((value) => !value)}><Palette size={14}/> Background</button>
        <button className="focus-ring inline-flex items-center gap-2 border border-white/20 px-3 py-2 font-mono text-[9px] uppercase tracking-[.08em] hover:bg-white/10" onClick={() => setAnnotations((value) => !value)}><Info size={14}/> Evidence</button>
      </div>
    </div>
    <div className="relative h-[420px] sm:h-[540px]">
      ...
      <div className="pointer-events-none absolute bottom-4 left-4 flex items-center gap-2 border border-ink bg-paper/95 px-3 py-2 font-mono text-[9px] uppercase tracking-[.1em] text-ink"><Box size={13}/> Unitless · one joined mesh · no moving parts</div>
    </div>
