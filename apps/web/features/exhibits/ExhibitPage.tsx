...
export function ExhibitPage({ demo = false }: { demo?: boolean }) {
  ...
  return <div className="min-h-screen paper-grain">
    ...
    <main>
      ...
      {!demo && <section className="folio-section border-y border-ink/25 bg-[#e8dfcf] text-ink"><FolioRail number="—" en="Editorial status" zh="编辑状态" locale={locale}/><div className="grid gap-8 px-6 py-9 md:px-10 lg:grid-cols-[1fr_2fr] lg:px-12"><div className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-[.1em]"><CircleAlert size={16}/>{t(locale,"Research register updated","研究登记册已更新")}</div><div><h2 className="font-display text-3xl sm:text-4xl">{t(locale,"97 records screened; evidence review continues.","已筛选 97 条记录，证据审核持续进行。")}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-quiet">{t(locale,"The register now includes 81 local PDFs and 9 open-licensed media records. No unreviewed historical conclusions have been published; the register is an inventory, not an endorsement of every statement in each source.","登记册现包含 81 份本地 PDF 与 9 项开放许可影像。项目尚未发布未经审核的历史结论；登记册是资料清单，不表示认可每个来源中的所有陈述。")}</p></div></div></section>}
      ...
      <section className="folio-section bg-night text-paper"><FolioRail number="04" en="Interpretive reconstruction" zh="解释性重建" locale={locale}/><div className="min-w-0 px-6 py-16 md:px-10 lg:px-12 lg:py-20"><div className="grid gap-8 border-t border-paper/70 pt-6 lg:grid-cols-[1.3fr_.7fr]"><h2 className="font-display text-5xl tracking-[-.04em] sm:text-6xl">{t(locale,"Interpretive Reconstruction","解释性重建")}</h2><p className="max-w-lg text-sm leading-6 text-fog">{t(locale,"A reconstruction is an evidence-based visual hypothesis, never a claim of exact form or manufacturing accuracy.","重建是以证据为基础的视觉假设，绝不宣称具有精确外形或制造级准确性。")}</p></div>{demo ? <div className="mt-12"><div className="mb-7 [&_p]:!text-fog [&>div]:!border-paper/40"><EvidenceLegend/></div><SafeProxyViewer/></div> : <div className="mt-12 border border-paper/25 p-5 sm:p-8"><div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr] lg:items-center"><div><span className="font-mono text-[10px] uppercase tracking-[.12em] text-redline">{t(locale,"Historical proxy gated","历史视觉代理受门禁")}</span><h3 className="mt-5 font-display text-4xl">{t(locale,"Evidence-bounded visual study","证据边界视觉研究")}</h3><p className="mt-4 text-sm leading-6 text-fog">{t(locale,"A live, nonfunctional study is available below so the page is not empty. It demonstrates the display language and continuous whole-object motion, but makes no claim about this draft exhibition's historical form.","下方提供实时、非功能性的视觉研究，页面不再留空。它展示视觉语言与整体连续运动，但不对本草稿展览的历史形态作出断言。")}</p></div><SafeProxyViewer mode="evidence-bounded"/></div></div>}<div className="mt-8 border border-redline"><p className="p-5 text-sm leading-6">{t(locale,"This reconstruction is a nonfunctional, evidence-based visual hypothesis. It is not a manufacturing model or an exact historical replica.","本重建是一个非功能性、以证据为基础的视觉假设，不是制造模型，也不是被宣称为完全准确的历史复制品。")}</p></div></div></section>
    </main>
